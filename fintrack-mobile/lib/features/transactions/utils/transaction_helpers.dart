...
List<TransactionUi> sorted(List<TransactionUi> list, TxSort sort) {
  ...
  switch (sort) {
    ...
    case TxSort.amountDesc:
      copy.sort((a, b) => b.amount.abs().compareTo(a.amount.abs())); // biggest RM first
      break;
    case TxSort.amountAsc:
      copy.sort((a, b) => a.amount.abs().compareTo(b.amount.abs())); // smallest RM first
      break;
  }
  return copy;
}
...
List<DateTime> monthsWithData(List<TransactionUi> list) {
  ...
  // ALWAYS include the current month so it's always selectable,
  // allowing the user to see the "no transactions yet" state for this month.
